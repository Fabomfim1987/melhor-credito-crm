...
// Label curto para chave MM/YYYY
...
function Linha({p,onFicha,onStatus}:{p:Parceiro;onFicha:()=>void;onStatus:(id:string,s:StatusCRM)=>void}) {
  ...
  const vsMedia=p.media_2025&&p.media_2025>0?((p.proj_prod-p.media_2025)/p.media_2025*100):null
  const meses=p.meses_display||[]
  const m0=meses[0]||{prod:p.mar_prod||0,dig:p.mar_dig||0}  // mês -2
  const m1=meses[1]||{prod:0,dig:0}                          // mês -1
  const m2=meses[2]||{prod:p.abr_prod||0,dig:p.abr_dig||0}  // mês atual
    ...
    <tr style={{borderBottom:'0.5px solid #f1f5f9',background:alerta?'#fffbeb':'#fff',transition:'background 0.15s'}}
      ...
      <td style={{padding:'9px 10px',textAlign:'right'}}>
        {ud&&(ud.prod>0||ud.dig>0)?(
          <>
            <div style={{fontSize:12,fontWeight:600,color:'#7c3aed'}}>{brl(ud.prod)}</div>
            <div style={{fontSize:10,color:'#94a3b8'}}>{brl(ud.dig)} dig.</div>
          </>
...
function AbaTabela({parceiros,lojaFiltro,loading,onUpdate}:{parceiros:Parceiro[];lojaFiltro?:string;loading:boolean;onUpdate:(p:Parceiro)=>void}) {
  ...
  // Calcular labels dos 3 meses para o header
  const hoje=new Date()
...
const CFGS=[
  ...
  {id:'base2025',label:'Base 2025',desc:'PRODUCAO_POR_EMPRESA_xxx_2025.xlsx',obrig:false},
]
...
function AbaUpload({onDadosAtualizados}:{onDadosAtualizados:()=>void}) {
  ...
  const processar=async()=>{
    ...
    try{
      ...
      if(data.ok){addLog(`✓ ${data.count} parceiros salvos!`);setResultado({ok:true,msg:`${data.count} parceiros atualizados!`});onDadosAtualizados()}
      ...
      <div style={{background:'#fff',borderRadius:10,border:'0.5px solid #e8ecf0',padding:'14px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:log.length?12:0}}>
          <div style={{fontSize:12,color:'#64748b'}}>
            {!temObrig?<span style={{color:'#d97706'}}>⚠ Adicione Nova Digitação e Produção</span>:`${Object.values(arqs).filter(a=>a.status==='ok').length} arquivo(s) · ${du}du · fator ${(DU_TOT/du).toFixed(2)}×`}
          </div>
          <button onClick={processar} disabled={!temObrig||processando}
            style={{display:'flex',alignItems:'center',gap:6,padding:'9px 18px',borderRadius:8,border:'none',background:temObrig&&!processando?'#07294a':'#e2e8f0',color:temObrig&&!processando?'#fff':'#94a3b8',cursor:temObrig&&!processando?'pointer':'default',fontSize:13,fontWeight:600}}>
