...
import { Search, X, ChevronDown, Send, User, RefreshCw, TrendingUp, AlertTriangle, Users, ArrowUpRight, MessageSquare, ChevronRight, Trash2, Upload, ExternalLink, FileSpreadsheet, CheckCircle, AlertCircle } from 'lucide-react'
    ...
    <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',justifyContent:'flex-end'}}>
      ...
      <div style={{position:'relative',width:'100%',maxWidth:520,background:'#fff',height:'100vh',display:'flex',flexDirection:'column',boxShadow:'-12px 0 48px rgba(0,0,0,0.15)',overflowY:'auto'}}>
          ...
          <div style={{padding:'12px 22px',borderBottom:'0.5px solid #f1f5f9',background:'#fafbff'}}>
            ...
            {vsMedia!==null&&(
              <div style={{marginTop:8,padding:'7px 11px',borderRadius:8,background:vsMedia>=0?'#f0fdf4':'#fff7ed',border:`0.5px solid ${vsMedia>=0?'#bbf7d0':'#fed7aa'}`}}>
                <span style={{fontSize:11,color:vsMedia>=0?'#15803d':'#c2410c',fontWeight:600}}>{vsMedia>=0?'▲':'▼'} Projeção abril {pct(vsMedia)} vs média 2025</span>
        ...
        <div style={{padding:'12px 22px',borderBottom:'0.5px solid #f1f5f9'}}>
          <div style={{fontSize:10,fontWeight:700,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:8}}>Março 2026 — Último mês completo</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
            <Num label="Digitado" val={p.mar_dig}/><Num label="Pago" val={p.mar_prod}/>
          </div>
          <div style={{fontSize:10,fontWeight:700,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:8}}>Abril ({p.du}du) — Esperado vs Real</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
            <Num label="Pro-rata Prod." val={p.prorata_prod} sub="Ritmo de março"/>
            <Num label="Abr. Prod. Real" val={p.abr_prod} sub={`Gap: ${p.gap_prod>=0?'+':''}${brl(p.gap_prod)}`} sc={gapClr(p.gap_prod)}/>
            <Num label="Pro-rata Dig." val={p.prorata_dig} sub="Ritmo de março"/>
            <Num label="Abr. Dig. Real" val={p.abr_dig} sub={`Gap: ${p.gap_dig>=0?'+':''}${brl(p.gap_dig)}`} sc={gapClr(p.gap_dig)}/>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            <Num label="Proj. Prod." val={p.proj_prod} sub={pct(p.var_prod)+' vs março'} sc={varClr(p.var_prod)}/>
            <Num label="Proj. Dig." val={p.proj_dig} sub={pct(p.var_dig)+' vs março'} sc={varClr(p.var_dig)}/>
          </div>
        </div>
...
function Linha({p,onFicha,onStatus}:{p:Parceiro;onFicha:()=>void;onStatus:(id:string,s:StatusCRM)=>void}) {
  ...
  return (
    <tr style={{borderBottom:'0.5px solid #f1f5f9',background:alerta?'#fffbeb':'#fff',transition:'background 0.15s'}}
      ...
      <td style={{padding:'9px 10px',textAlign:'right'}}>
        <div style={{fontSize:12,fontWeight:600,color:'#0f172a'}}>{brl(p.mar_prod)}</div>
        <div style={{fontSize:10,color:'#94a3b8'}}>{brl(p.mar_dig)} dig.</div>
      </td>
      <td style={{padding:'9px 10px',textAlign:'right'}}>
        <div style={{fontSize:12,fontWeight:600,color:'#0f172a'}}>{brl(p.abr_prod)}</div>
        <div style={{fontSize:10,color:'#94a3b8'}}>{brl(p.abr_dig)} dig.</div>
      </td>
      <td style={{padding:'9px 10px',textAlign:'right'}}>
      ...
      <td style={{padding:'9px 10px',textAlign:'right'}}>
        ...
        <div style={{fontSize:10,fontWeight:600,color:vsMedia!==null?varClr(vsMedia):varClr(p.var_prod)}}>
          {vsMedia!==null?`${pct(vsMedia)} vs média`:`${pct(p.var_prod)} vs mar`}
        </div>
      </td>
...
function AbaTabela({parceiros,lojaFiltro,loading,onUpdate}:{parceiros:Parceiro[];lojaFiltro?:string;loading:boolean;onUpdate:(p:Parceiro)=>void}) {
  ...
  const updateStatus=(id:string,s:StatusCRM)=>{
    // update local handled by parent
  }
  const filtrados=parceiros.filter(p=>{
    if (lojaFiltro) {
      // Pedro: mostrar sala PEDRO da Nova + todos parceiros GLM
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      <div style={{background:'#fff',borderRadius:10,padding:'11px 14px',border:'0.5px solid #e8ecf0',display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        ...
        {!lojaFiltro&&[{label:'Promotora',val:fProm,set:setFProm,opts:promotoras}].map(f=>(
        ...
        <span style={{fontSize:12,color:'#94a3b8',marginLeft:'auto'}}>{filtrados.length} de {lojaFiltro==='PEDRO'?parceiros.filter(p=>p.loja==='PEDRO'||p.promotora==='GLM').length:lojaFiltro?parceiros.filter(p=>p.loja===lojaFiltro).length:parceiros.length}</span>
      </div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead>
              <tr style={{background:'#f0f4f8',borderBottom:'2px solid #e2e8f0'}}>
                {[['Parceiro','left'],['Q','center'],['Média 2025 / Pico','right'],['Mar. 2026','right'],['Abr. Real','right'],['Gap','right'],['Projeção','right'],['Status CRM','left'],['Obs','center'],['','center']].map(([h,a])=>(
            ...
            <tbody>
              {loading?<tr><td colSpan={10} style={{padding:'48px',textAlign:'center',color:'#94a3b8'}}>Carregando...</td></tr>
              :filtrados.length===0?<tr><td colSpan={10} style={{padding:'48px',textAlign:'center',color:'#94a3b8'}}>Nenhum parceiro encontrado.</td></tr>
...
function AbaGerencial({parceiros,loading}:{parceiros:Parceiro[];loading:boolean}) {
  ...
  const pctMeta=Math.min(totalProj/META_ABRIL*100,100)
  const corMeta=totalProj>=META_ABRIL?'#15803d':totalProj>=META_ABRIL*0.8?'#b45309':'#dc2626'
  ...
  const alertasCinzas: {id:string;msg:string;sub:string}[]=[]

  // Vermelho: negociações paradas +5 dias
  ...
  // Amarelo: projeção abaixo da meta
  if(totalProj<META_ABRIL*0.9) alertasAmarelos.push({id:'meta',msg:`Projeção ${pct(((totalProj-META_ABRIL)/META_ABRIL)*100)} da meta`,sub:`Faltam ${brl(META_ABRIL-totalProj)} para atingir R$ 105k`})
  // Cinza: gestores sem atualizar hoje
  const GESTORES_KEY=['VALERIA','PEDRO','ERALDO']
  GESTORES_KEY.forEach(key=>{
    const gp=parceiros.filter(p=>p.loja===key)
    const datas=gp.flatMap(p=>p.observacoes?.map(o=>o.data)||[])
    if(!datas.length){alertasCinzas.push({id:`sem-obs-${key}`,msg:`${key.charAt(0)+key.slice(1).toLowerCase()} sem nenhuma observação registrada`,sub:'Nenhuma interação registrada'}); return}
    const ultima=new Date(Math.max(...datas.map(d=>new Date(d).getTime())))
    const diff=Math.floor((Date.now()-ultima.getTime())/1000)
    if(diff>86400) alertasCinzas.push({id:`old-${key}`,msg:`${key.charAt(0)+key.slice(1).toLowerCase()} não atualiza há mais de 24h`,sub:`Última obs: ${ultima.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})} ${ultima.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`})
  })
    ...
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      <div style={{background:'#fff',borderRadius:12,padding:'14px 18px',border:'0.5px solid #e8ecf0',boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:8}}>
          <div>
            <div style={{fontSize:9,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.06em',fontWeight:600,marginBottom:3}}>Meta Abril 2026</div>
            <div style={{display:'flex',alignItems:'baseline',gap:8}}>
              <span style={{fontSize:22,fontWeight:700,color:corMeta,letterSpacing:'-0.5px'}}>{brl(totalProj)}</span>
              <span style={{fontSize:13,color:'#94a3b8'}}>de {brl(META_ABRIL)}</span>
          ...
          <div style={{textAlign:'right'}}>
            ...
            <div style={{fontSize:16,fontWeight:700,color:totalProj>=META_ABRIL?'#15803d':'#dc2626'}}>{totalProj>=META_ABRIL?'✓ Meta atingida':brl(META_ABRIL-totalProj)}</div>
          </div>
        </div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:5}}>
          ...
          <span style={{fontSize:9,color:'#94a3b8',fontWeight:600}}>Meta: R$ 105k</span>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10}}>
        {[
          {label:'Média 2025',val:brl(comMedia.length>0?totalMedia/comMedia.length:0),sub:`${comMedia.length} parceiros com histórico`,color:'#4f46e5',bg:'#ede9fe',icon:<TrendingUp style={{width:15,height:15}}/>},
          {label:'Março 2026',val:brl(totalMar),sub:'Último mês completo',color:'#07294a',bg:'#dbeafe',icon:<Users style={{width:15,height:15}}/>},
          {label:'Abr. Realizado',val:brl(totalAbr),sub:'Acumulado no período',color:'#0369a1',bg:'#e0f2fe',icon:<TrendingUp style={{width:15,height:15}}/>},
          {label:'Projeção Abril',val:brl(totalProj),sub:vsMedia2025!==null?`${pct(vsMedia2025)} vs média 2025`:pct(totalMar>0?(totalProj-totalMar)/totalMar*100:null),color:totalProj>=totalMar?'#15803d':'#dc2626',bg:totalProj>=totalMar?'#dcfce7':'#fee2e2',icon:<ArrowUpRight style={{width:15,height:15}}/>},
          ...
          <div key={k.label} style={{background:'#fff',borderRadius:12,padding:'14px 16px',border:'0.5px solid #e8ecf0',display:'flex',gap:12,alignItems:'flex-start',boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
            <div style={{width:38,height:38,borderRadius:10,flexShrink:0,background:(k as any).bg,color:k.color,display:'flex',alignItems:'center',justifyContent:'center'}}>{k.icon}</div>
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:20,fontWeight:700,color:k.label==='Projeção Abril'||k.label==='Ação Necessária'?k.color:'#0f172a',lineHeight:1,letterSpacing:'-0.5px'}}>{k.val}</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
        {GESTORES.map(g=>{
          ...
          const mar=gpNova.reduce((a,p)=>a+p.mar_prod,0)
            ...
            <div key={g.key} style={{background:'#fff',borderRadius:12,border:'0.5px solid #e8ecf0',overflow:'hidden',boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
              <div style={{padding:'11px 14px 9px',borderBottom:'0.5px solid #f1f5f9',display:'flex',alignItems:'center',justifyContent:'space-between',background:obsAtrasada?'#fffbeb':'#fff'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:3}}>
                  ...
                  {tendencia!==null&&<span style={{fontSize:10,fontWeight:700,color:tendencia>=0?'#15803d':'#dc2626'}}>{tendencia>=0?'↑':'↓'} {Math.abs(tendencia).toFixed(0)}% vs mar</span>}
                </div>
              </div>
              <div style={{padding:'10px 14px',display:'flex',flexDirection:'column',gap:5}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between'}}><span style={{fontSize:10,color:'#64748b'}}>Projeção produção</span><span style={{fontSize:13,fontWeight:700,color:proj>=mar?'#15803d':'#dc2626'}}>{brl(proj)}</span></div>
      ...
      {/* ALERTAS COM PRIORIDADE */}
      {(alertasVermelhos.length>0||alertasAmarelos.length>0||alertasCinzas.length>0)&&(
          ...
          <div style={{display:'flex',flexDirection:'column',gap:5}}>
            {alertasVermelhos.map(a=>(
              <div key={a.id} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'9px 12px',borderRadius:8,background:'#fef2f2',border:'0.5px solid #fecaca'}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:11,fontWeight:600,color:'#7f1d1d'}}>{a.msg}</div>
            ...
            {alertasAmarelos.map(a=>(
              <div key={a.id} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'9px 12px',borderRadius:8,background:'#fffbeb',border:'0.5px solid #fde68a'}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:11,fontWeight:600,color:'#78350f'}}>{a.msg}</div>
                  <div style={{fontSize:10,color:'#94a3b8',marginTop:1}}>{a.sub}</div>
                </div>
            ...
            {alertasCinzas.map(a=>(
              <div key={a.id} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'9px 12px',borderRadius:8,background:'#f8fafc',border:'0.5px solid #e2e8f0'}}>
                <div style={{width:8,height:8,borderRadius:'50%',background:'#94a3b8',flexShrink:0,marginTop:3}}/>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,fontWeight:500,color:'#374151'}}>{a.msg}</div>
                  <div style={{fontSize:10,color:'#94a3b8',marginTop:1}}>{a.sub}</div>
                </div>
                <span style={{fontSize:9,color:'#94a3b8',background:'#f1f5f9',padding:'2px 7px',borderRadius:10,whiteSpace:'nowrap',flexShrink:0}}>⚪ Monitorar</span>
              </div>
            ))}
          </div>
        </div>
...
function calcS(mar:number,abr:number,fator:number,aus=false):[string,number]{
...
const CFGS=[
  ...
  {id:'glm',label:'GLM — Análise',desc:'Arquivo Export da GLM',obrig:false},
  ...
  return (
    <div style={{background:ok?'#f0fdf4':err?'#fef2f2':drag?'#eff6ff':'#fff',border:`0.5px solid ${ok?'#86efac':err?'#fca5a5':drag?'#07294a':'#e2e8f0'}`,borderRadius:9,padding:'11px 13px',cursor:arq?'default':'pointer',transition:'all 0.1s'}}
      ...
      {ok?<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        ...
        <div style={{fontSize:10,color:'#64748b',marginTop:2}}>{arq!.rows} linhas lidas</div>
      </div>
      :err?<div>
...
function AbaUpload({onDadosAtualizados}:{onDadosAtualizados:()=>void}) {
  ...
  const processar=async()=>{
    ...
    if(nd&&np){
      ...
      nd.slice(1).forEach((r:any[])=>{const nr=parseInt(r[0]);if(!isNaN(nr))dM[nr]=[String(r[3]||''),parseFloat(r[8])||0,String(r[1]||'')]})
    ...
    const ld=arqs['lev_dig']?.data;const lp=arqs['lev_prod']?.data
    if(ld||lp){
      addLog('Lev: processando...')
      const levD:Record<string,[string,number]>={};const levP:Record<string,number>={};const levN:Record<string,string>={}
      if(ld)ld.slice(4).forEach((r:any[])=>{if(!r[0]||String(r[0]).startsWith('Total'))return;const m=String(r[0]).match(/\|\s*(\d+)/);if(!m)return;const cod=m[1];levD[cod]=[String(r[0]).split('|')[0].trim(),parseBRL(r[3])];levN[cod]=String(r[0]).split('|')[0].trim()})
      if(lp)lp.slice(3).forEach((r:any[])=>{if(!r[0])return;const m=String(r[0]).match(/\[(\d+)\]/);if(!m)return;const cod=m[1];levP[cod]=(levP[cod]||0)+(parseFloat(r[12])||0);if(!levN[cod])levN[cod]=String(r[0]).replace(/\s*\[.*?\]/g,'').replace(/\xa0/g,'').trim()})
      new Set([...Object.keys(levD),...Object.keys(levP)]).forEach(cod=>{
        const nome=levN[cod]||`COD ${cod}`;const ad=levD[cod]?.[1]||0;const ap=levP[cod]||0
        const[sd,pd_]=calcS(0,ad,fator);const[sp,pp]=calcS(0,ap,fator)
        parceiros.push({id:`lev-${cod}`,nr:cod,nome,loja:'LEV',promotora:'Lev',du,fator:Math.round(fator*100)/100,
          mar_dig:0,mar_prod:0,prorata_dig:0,prorata_prod:0,
          abr_dig:Math.round(ad*100)/100,abr_prod:Math.round(ap*100)/100,
          gap_dig:Math.round(ad*100)/100,gap_prod:Math.round(ap*100)/100,
          proj_dig:Math.round(pd_*100)/100,proj_prod:Math.round(pp*100)/100,
    ...
    const base=arqs['base2025']?.data
    if(base){
      addLog('Base 2025: cruzando histórico...')
      const bIdx:Record<string,any[]>={}
      base.slice(1).filter((r:any[])=>r[0]).forEach((r:any[])=>{bIdx[normStr(String(r[1]||''))]=r})
      let matched=0
      parceiros.forEach(p=>{
        const n=normStr(p.nome);let hit=bIdx[n]
        if(!hit){const ws=n.split(' ').filter((w:string)=>w.length>4);let best:any[]|null=null;let bs=0
          Object.entries(bIdx).forEach(([k,v])=>{const sc=ws.filter((w:string)=>k.includes(w)).length;if(sc>=2&&sc>bs){best=v;bs=sc}})
          if(best)hit=best}
        if(hit){p.media_2025=parseBRL(hit[16])||null;p.pico_2025=parseBRL(hit[17])||null;p.pico_mes=String(hit[18]||'')||null;p.perfil_2025=String(hit[19]||'')||null;p.total_2025=parseBRL(hit[14])||null;p.meses_ativos=parseInt(hit[15])||null;matched++}
      })
      addLog(`Base 2025: ${matched}/${parceiros.length} enriquecidos`)
    }
    if(parceiros.length===0){setResultado({ok:false,msg:'Nenhum arquivo válido. Adicione pelo menos Nova Digitação + Nova Produção.'});setProcessando(false);return}
    addLog(`Salvando ${parceiros.length} parceiros...`)
    try{
      const res=await fetch('/api/parceiros',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({parceiros_bulk:parceiros})})
      const data=await res.json()
      if(data.ok){addLog(`✓ ${data.count} parceiros salvos! Status CRM e observações preservados.`);setResultado({ok:true,msg:`${data.count} parceiros atualizados!`});onDadosAtualizados()}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      <div style={{background:'#fff',borderRadius:10,border:'0.5px solid #e8ecf0',padding:'14px 16px'}}>
        ...
        <div style={{fontSize:12,color:'#64748b',lineHeight:1.6,marginBottom:12}}>Status CRM e observações são preservados automaticamente.</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9}}>
        {CFGS.map(cfg=><DropZ key={cfg.id} label={cfg.label} desc={cfg.desc} obrig={cfg.obrig} arq={arqs[cfg.id]} onChange={files=>onDrop(cfg.id,files)} onRemove={()=>setArqs(prev=>{const n={...prev};delete n[cfg.id];return n})}/>)}
      </div>
      <div style={{background:'#fff',borderRadius:10,border:'0.5px solid #e8ecf0',padding:'14px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:log.length?12:0}}>
          <div style={{fontSize:12,color:'#64748b'}}>
            {!temObrig?<span style={{color:'#d97706'}}>⚠ Adicione pelo menos Nova Digitação e Nova Produção</span>
              :`${Object.values(arqs).filter(a=>a.status==='ok').length} arquivo(s) prontos · ${du}du · fator ${(DU_TOT/du).toFixed(2)}×`}
          </div>
          <button onClick={processar} disabled={!temObrig||processando}
            style={{display:'flex',alignItems:'center',gap:6,padding:'9px 18px',borderRadius:8,border:'none',
              background:temObrig&&!processando?'#07294a':'#e2e8f0',color:temObrig&&!processando?'#fff':'#94a3b8',
    ...
    <div style={{minHeight:'100vh',background:'#f0f2f5',fontFamily:'var(--font-sans,system-ui)'}}>
      ...
      <div style={{background:'#07294a',color:'#fff',padding:'0 20px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:50,boxShadow:'0 4px 16px rgba(0,0,0,0.25)'}}>
          ...
          <div style={{display:'flex',gap:2}}>
            {TABS.map(t=>(
              <button key={t.id} onClick={()=>setTab(t.id)}
                style={{padding:'6px 13px',fontSize:11.5,borderRadius:6,border:'none',cursor:'pointer',
                  fontWeight:tab===t.id?600:400,letterSpacing:tab===t.id?'-0.1px':'0',
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{fontSize:10,color:'rgba(255,255,255,0.4)',letterSpacing:'0.02em'}}>{new Date().toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric'})}</div>
      ...
      <div style={{maxWidth:1600,margin:'0 auto',padding:'16px 20px'}}>
        ...
        <div style={{marginTop:10,fontSize:11,color:'#94a3b8',textAlign:'center'}}>Melhor Crédito · Nova + GLM + Lev · Base 2025</div>
      </div>
    </div>
  )
}
